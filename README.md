...
Prophet 모델 기반의 시계열 분석으로 계절성, 요일 패턴, 프로모션 효과를 반영.
정확한 수요량을 사전 계산해 재고 회전율 향상 및 낭비 최소화 달성.

#### 자동 발주
예측 결과와 안전재고 규칙을 기반으로 점포별 최적 발주량을 자동 계산 및 제안.
“추천 반영” 한 번으로 발주서를 자동 생성할 수 있어 업무 속도 향상.
AI 제안값의 근거(예측치, 안전재고, 최소/최대 발주수량)를 함께 표시해 신뢰성 있는 의사결정 지원.
...
| 발주 관리 | AI 자동발주 및 수동 발주 통합 | AI 예측을 반영한 자동 생성, 공급사 승인/반려 및 출고 연동 |
| 주문 관리 | 가맹점 주문·승인·정산 일원화 | 주문 생성, 검수, 정산의 과정 제공 |
| 재고/창고 관리 | 입·출고 및 안전재고 규칙 | 창고별 입출고 이력 관리, 안전재고 임계값 이하 시 경고 표시 |
| 수요 예측 | AI 수요 예측 | 과거 판매·발주·재고 데이터를 학습하여 SKU×지점별 단기·중기 수요를 예측. |
