...
public class ShipmentInventoryListener {
    ...
    public void applyInventory(ShipmentDeliveredEvent event) {
        ...
        if (Boolean.TRUE.equals(shipment.getInventoryApplied())) {
            log.info("already applied: shipmentId={}", event.shipmentId());
            return;
        }
        ...
        try {
            ...
            lines.forEach(d -> {
                ...
                inventory.increaseOnHand(d.getOrderQty().intValue());
            ...
            log.info("store inventory applied: shipmentId={}, storeOrderId={}",
